...
namespace BrainFailProductions.PivotModder
{
    ...
    public class InspectorDrawer : Editor
    {
        ...
        private const string ICON_PATH = "Assets/PivotModder/icons/";
        ...
        public void OnDestroy()
        {
            ...
            if (Application.isEditor && thisGameObject == null && !Application.isPlaying)
            {
                ...
                try
                {
                    key = thisGameObject.GetHashCode();
        ...
        public override void OnInspectorGUI()
        {
            ...
            string path = ICON_PATH + "icon.png";
            ...
            if (GUILayout.Button("<size=13><b><color=#A52A2AFF>PIVOT MODDER</color></b></size>  <size=7><b><color=#A52A2AFF>v3.40</color></b></size>", style)) { toolMainFoldout = !toolMainFoldout; }
            ...
            var p = ICON_PATH + "help.png";

            content.image = EditorGUIUtility.Load(p) as Texture;
            ...
            {
                Application.OpenURL("https://brainfailproductions.000webhostapp.com/pivotmodder_runtime_api_docs/html/class_pivot_modder_runtime_1_1_pivot_modder_runtime.html");
            }
            ...
            if (toolMainFoldout)
            {
                ...
                if (isPositionSelected)
                {
                    path = ICON_PATH + "rot.png";
                    content.tooltip = "Switch to rotation mode?";
                }
                else
                {
                    path = ICON_PATH + "pos.png";
                ...
                if (didPress && allowPivotMod)
                {
                    ...
                    if (!isMeshlessObject)
                    {
                        toModify = Selection.activeTransform.GetComponent<MeshFilter>().sharedMesh;

                        if (toModify != null && !toModify.isReadable)
                        {
                            string assetPath = AssetDatabase.GetAssetPath(toModify);

                            //UtilityServices.ChangeMeshReadability(toModify, true, false);
                            EditorUtility.DisplayDialog("Mesh Unreadable", $"Mesh \"{toModify.name}\" is not readible. Please mark it readible for the pivot modification process to complete. You can mark this mesh Read/Write enabled in the model import settings for the model at path \"{assetPath}\". For your convenience we have selected the model for you. ", "ok");
                            Selection.activeGameObject = AssetDatabase.LoadAssetAtPath<GameObject>(ModelImporter.GetAtPath(assetPath).assetPath);
                            GUIUtility.ExitGUI();
                        }
                    ...
                    else if(!isMeshlessObject && UnityEditor.Experimental.SceneManagement.PrefabStageUtility.GetCurrentPrefabStage() != null)
                    {
                        ...
                        if(m != null)
                        {
                            if(AssetDatabase.Contains(m))
                            {
                                if(AssetDatabase.GetAssetPath(m).ToLower().StartsWith("library"))
                                {
                                    //Inbuilt asset must save
                                    EditorUtility.DisplayDialog("Warning", "Pivot changes have been successfully applied to the prefab instance. However, please remember to save the mesh before saving the prefab and exiting prefab mode. Otherwise you'll get undesired consequences!", "ok");
                                }

                                else
                                {
                                    // do something if not associated with a model

                                    ModelImporter imp = ModelImporter.GetAtPath(AssetDatabase.GetAssetPath(m)) as ModelImporter;
                    ...
                    if (!isMeshlessObject && toModify != null) { EditorUtility.SetDirty(Selection.activeGameObject); }
                    ...
                    //if(wasUnreadable)
                    //{
                        //Debug.LogWarning($"Mesh \"{toModify.name}\" was not readible so we marked it readible for the pivot modification process to complete and changed it back to non-readible after completion. This process can slow down pivot modification. You may want to mark this mesh Read/Write enabled in the model import settings, so that next time pivot modification on this model can be faster.");
                        //UtilityServices.ChangeMeshReadability(toModify, false, false);
                    //}
...
#region X INCREMENT REGION
                ...
                content.image = EditorGUIUtility.Load(ICON_PATH + "dec.png") as Texture;
                ...
                content.image = EditorGUIUtility.Load(ICON_PATH + "inc.png") as Texture;
