	...
	public class SceneManipulator
	{
		...
		public bool DoMouseDown(PickMode initialMode)
		{
			...
			else if (currentToolSelection == ToolSelection.Pipette)
			{
				...
				int hitTriIndex = -1;
				if (targetMeshCollider != null && RaycastUtil.Raycast(targetMeshCollider, targetVertices, targetTriangles, pickRay, out hitTriIndex, 10000.0f))
				{
		...
		private PickMode PickTriangle(PickMode pickMode)
		{
			...
			int hitTriIndex = -1;
			if (RaycastUtil.Raycast(targetMeshCollider, targetVertices, targetTriangles, pickRay, out hitTriIndex, 10000.0f))
			{
